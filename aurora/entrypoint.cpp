...
struct ExampleAppConsole {
    ...
    void exec_command(std::string const& command_line) {
        items.push_back(std::format("# {}", command_line));
        ...
        if (command_line == "clear") {
            ...
            for (int i = 0; i < Commands.Size; i++)
                items.push_back(std::format("- {}", Commands[i]));
        }
            ...
            items.push_back(std::format("Unknown command: `{}`\n", command_line));
        }
...
void tools_hasher(bool& aOpen, std::unordered_set<std::string> const& aFileList) {
    ...
    if (ImGui::Begin("Hasher", &aOpen)) {
        if (ImGui::InputText("Input", &input)) {
            ...
            exists = aFileList.contains(std::format("{:x}.pc", output));
        }

        ImGui::TextUnformatted(std::format("0x{:x}", output).c_str());
...
void tools_binary_search(bool& aOpen) {
    ...
    mem_edit_1.DrawWindow("Memory Editor", file.data(), file.size());
    ...
    if (ImGui::Begin("Binary Search", &aOpen)) {
                ...
                future = std::async(std::launch::async, [](std::string input) {
...

                    {
                        ...
                        while (true) {
                            ...
                            result.matches.emplace_back("THUMPER", std::distance(bytes.begin(), it), std::distance(bytes.begin(), it) + parsed.size());
        ...
        if (future) {
            ...
            ImGui::Text("%d results for %s", matches.matches.size(), matches.pattern.c_str());

            for (auto const& match : matches.matches) {
                std::string str = std::format("{} @ 0x{:x} -> 0x{:x}", match.file, match.start, match.end);
                if (ImGui::Selectable(str.c_str())) {
                    
                    std::string path = std::format("{}/cache/{}", kThumperDirectory, match.file);
...
void validate_executables() {
    ...
    std::string baseExePath = std::format("{}/{}", kThumperDirectory, "THUMPER_win8.exe");
    std::string unpackedExePath = std::format("{}/{}", kThumperDirectory, "THUMPER_win8.exe.unpacked.exe");
...
void cache_scan(std::unordered_set<std::string>& pcFileStorage) {
    ...
    console.items.push_back(std::format("{} file(s)", pcFileStorage.size()));
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
    // Load configs
    {
        ...
        if (luaL_dofile(L, "config.lua") != LUA_OK) {
            console.items.push_back(std::format("Lua Error: {}", lua_tostring(L, -1)));
        }
        else {
            if (!lua_istable(L, -1)) {
	...
	GLFWwindow* window = glfwCreateWindow(1280, 720, "Aurora v0.0.4-a.1", nullptr, nullptr);
