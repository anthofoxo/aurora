...
void aurora::register_plugin_api(lua_State* L) {
	...
	lua_pushcfunction(L, [](lua_State* L) -> int {
		...
		lua_pushinteger(L, 1);
		lua_rawget(L, 4);
		value[0] = static_cast<float>(lua_tonumber(L, -1));
		lua_pop(L, 1);

		if (buffer == GL_COLOR) {
			lua_pushinteger(L, 2);
			lua_rawget(L, 4);
			value[1] = static_cast<float>(lua_tonumber(L, -1));
			lua_pop(L, 1);
			lua_pushinteger(L, 3);
			lua_rawget(L, 4);
			value[2] = static_cast<float>(lua_tonumber(L, -1));
			lua_pop(L, 1);
			lua_pushinteger(L, 4);
			lua_rawget(L, 4);
			value[3] = static_cast<float>(lua_tonumber(L, -1));
			lua_pop(L, 1);
		}
	...
	lua_newtable(L);
#define AU_IMPL_GL_EXPAND(L, name) \
	lua_pushinteger(L, GL_##name); \
