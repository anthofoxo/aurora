...
struct Serializer {
	...
	template <typename T>
	inline void serialize(char const* aField, std::optional<T>& aValue, bool aCondition) {
		if (aCondition) {
			if (opt_begin())
				aValue = T();
			else
				assert(aValue.has_value());

			T& val = *aValue;
			serialize(aField, val);
		} else {
			aValue = std::nullopt;
		}
	}
	...
	template <typename T>
	inline void serialize(char const* aField, std::vector<T>& aValue) {
...
struct SerializerWriterLua final : public Serializer {
	...
	inline void array_end(char const* aField) override {
