...
S_API bool S_CALLTYPE SteamAPI_RestartAppIfNecessary(uint32 unOwnAppID) {
	...
	aurora::main();
	if (!aurora::should_launch_thumper()) { std::exit(0); }
...
S_API void* S_CALLTYPE SteamInternal_FindOrCreateUserInterface(HSteamUser hSteamUser, const char* pszVersion) { return STEAM_FORWARD(SteamInternal_FindOrCreateUserInterface, hSteamUser, pszVersion); }
...
S_API void S_CALLTYPE SteamAPI_UnregisterCallResult(class CCallbackBase* pCallback, SteamAPICall_t hAPICall) { return STEAM_FORWARD(SteamAPI_UnregisterCallResult, pCallback, hAPICall); }
